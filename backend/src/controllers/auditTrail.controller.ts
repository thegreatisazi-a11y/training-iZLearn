...
import { formatDateTime, toEndBound, formatStamp } from '../utils/dateUtils';
...
export const exportAudit = asyncHandler(async (req: Request, res: Response) => {
  ...
  // IST (the configured display tz) — toLocaleString() would follow the SERVER clock, UTC on Render.
  const stamp = formatStamp(new Date(), org.timezone);
  ...
  if (format === 'xls' || format === 'xlsx') {
    // Built with the same buffered helper as every other Excel export on the platform, so the
    // layout (title row, bold header, autofilter, "Generated by …" footer) matches them.
    //
    // This replaced ExcelJS's STREAMING WorkbookWriter piped straight into the response, which
    // produced a file Excel rejected with "the file format or file extension is not valid" — the
    // audit trail was the only export using that path, and the only one that failed. Buffering
    // also restores a real Content-Length, so the download shows a progress percentage.
    //
    // Buffering means the sheet must be bounded. Excel itself stops at ~1,048,576 rows, so the cap
    // below is the practical limit either way — and when it bites it is stated IN the sheet rather
    // than silently truncating. CSV above remains the unbounded, streamed export for the full record.
    // Same cap (and same env override) as the Reports Excel export — see report.service.ts.
    const XLSX_CAP = parseInt(process.env.XLSX_MAX_ROWS || '100000', 10);
    const rows: Array<Record<string, unknown>> = [];
    let truncated = false;
    for await (const row of streamRows()) {
      if (rows.length >= XLSX_CAP) {
        truncated = true;
        break;
      }
      rows.push(row);
    ...
    const title = truncated
      ? `Audit Trail Report — first ${XLSX_CAP.toLocaleString()} rows (filtered set is larger; export as CSV for the complete record)`
      : 'Audit Trail Report';
    const buffer = await exportToExcel(
      COLUMNS.map((c) => ({ key: c.key, header: c.header, width: 22 })),
      rows,
      'Audit Trail',
      title,
      { generatedBy, timezone: org.timezone },
    );
  ...
  try {
    ...
    // Log the real reason: swallowing it made a crashed browser indistinguishable from Chromium
    // not being installed, and the message below sent us looking at the wrong thing.
    logger.error(`Audit trail PDF export failed: ${(e as Error)?.message ?? e}`);
