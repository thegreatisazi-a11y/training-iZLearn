...
export async function listForms(q: PaginationQuery & { topicId?: string }) {
  ...
  // MongoDB has no relational join, so resolve each form's course name (and number)
  // from its topicId in one batched lookup — the UI shows the course name, not the id.
