...
import { formatStamp } from './dateUtils';
...
export async function exportToExcel(
  ...
  sheetName = 'Report',
  titleRow?: string,
  meta?: { generatedBy?: string; timezone?: string },
): Promise<Buffer> {
  ...
  ws.addRow([]);
  // IST (or the configured tz) — see the note in csvExporter: toLocaleString() follows
  // the SERVER's timezone, which is UTC on the deployment host.
  const stamp = formatStamp(new Date(), meta?.timezone);
